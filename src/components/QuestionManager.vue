                ...
                <div class="flex-1 overflow-hidden flex">
                  ...
                  <div v-show="editorTab === 'preview' || editorTab === 'split'"
                    ...
                    <div class="p-4 sm:p-6">
                      <div v-if="editForm.content" class="prose-content" v-html="renderedPreview" @click="handleCodeCopyClick"></div>
...
import { renderMarkdown, handleCodeCopyClick } from '@/composables/markdownRenderer'
...
// Markdown 预览：与题目详情页共用同一个渲染器，避免两边逻辑漂移
const renderedPreview = computed(() => {
  if (!editForm.value.content) return ''
  // 编辑预览里不渲染流程图（避免频繁输入时反复布局），但保留复制按钮
  return renderMarkdown(editForm.value.content, { mermaid: false, copyButton: true })
})
